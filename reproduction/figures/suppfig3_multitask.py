...
for task in tasks:
    with open (f"../test_logs/test7/{task}/c_indices.txt", "r") as f:
        for lines in f:
...
for task in tasks:
    with open (f"../test_logs/test7/{task}/auroc.txt", "r") as f:
        for lines in f:
...
for task in tasks:
    with open (f"../test_logs/test7/{task}/aupr.txt", "r") as f:
        for lines in f:
...
ax1.set_position([box.x0 - box.width * 0.1, box.y0, box.width, box.height])
plt.tight_layout()
plt.savefig("suppfig3_multitask.png")
plt.savefig("suppfig3_multitask.pdf")
#db.set_trace()