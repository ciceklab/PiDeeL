#!/usr/bin/env python3
"""
Supplementary Figure 10: PC-Hazard Loss Comparison

Checks for required logs from test30 (PC-Hazard loss experiments).
"""
import subprocess
import sys
...
LOG_CHECKS = [
    ("test_logs/test30/2layer/no_pathway/c_indices.txt", "test_scripts/test30/2layer/no_pathway/main.py"),
    ("test_logs/test30/3layer/no_pathway/c_indices.txt", "test_scripts/test30/3layer/no_pathway/main.py"),
...
def plot_figure():
    ...
    script = REPO_ROOT / "figures" / "suppfig10_pchazard.py"
    try:
        subprocess.run([sys.executable, str(script)], check=True, cwd=str(script.parent))
        print("\n✓ Supp Fig 10 generated!")
        print("  Output: figures/suppfig10_pchazard.png, suppfig10_pchazard.pdf")
        return True
    except subprocess.CalledProcessError as e:
        print(f"✗ Failed: {e}")
        return False
...
if __name__ == "__main__":
    print("="*60 + "\nSupp Fig 10 Runner: PC-Hazard Comparison\n" + "="*60)
