...
from load_targeted_data import predicted_quant, events, survival,pathway_info, tumor_grade, types
...
for index, item in enumerate(types):
    if item == "remove":
        ...
        this = str(item) + str(tumor_grade[index])
        dicti[this] = []
unique_types = list(dicti.keys())
print(unique_types)
type_grade = []
for index, item in enumerate(types):
    if item == "remove":
        type_grade.append("unknown")
    else:
        type_grade.append(str(item) + str(tumor_grade[index]))
type_grade = np.array(type_grade)
c_indices = []
for SEED in SEEDS:
    ...
    for i in range(len(result)):
        ...
        type_grade_train = type_grade[result[i][0]]
        type_grade_test = type_grade[result[i][1]]
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.savefig(f"/home/gunkaynar/targeted_survival/plots/{task}/{SEED}_{i}.png")
        plt.clf()
        ...
        #_ = model1.compute_baseline_hazards(input=total_features,target=(total_labels[:,1],total_labels[:,0]))


        #compute c index for each type in test set
        for item in set(type_grade):
            indices = [i for i, x in enumerate(type_grade_test) if x == item]
            if indices:
                features_test_item = features_test[indices].float().to(device)
                labels_test_item = labels_test[indices].float().to(device)
                surv = model1.predict_surv_df(features_test_item)
                ev = EvalSurv(surv,  y_test[:,1][indices], y_test[:,0][indices], censor_surv='km')
                try:
                    dicti[item].append(ev.concordance_td())
