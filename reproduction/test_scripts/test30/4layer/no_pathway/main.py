...
from load_targeted_data import predicted_quant, events, survival
...
for SEED in SEEDS:
    ...
    for i in range(len(result)):
        ...
        model = Net(X_train.shape[1],num_durations).to(device)
        ...
        for epoch in range(1, epochnum+1):
            ...
            loss = criterion(pred_probas, labels[0], labels[1],labels[2]) 
            ...
            with torch.no_grad():
                ...
                loss = criterion(pred_probas, labels_cv[0], labels_cv[1],labels_cv[2])
                vald_loss += loss.item()
        ...
        model1 = PCHazard(model, tt.optim.Adam)
        ...
        #torch.save(model.state_dict(), f"{SEED}_{i}net.pth")
        


with open(f"../../../../test_logs/test30/{task}/c_indices.txt" ,"w") as f:
