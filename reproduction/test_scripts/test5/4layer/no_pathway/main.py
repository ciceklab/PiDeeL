...
from load_targeted_data import predicted_quant,ages,grade
from model import Net, initialize_weights
from config import batchsize, learningrate, epochnum, task, SEEDS, l2_lambda, patience
...
for SEED in SEEDS:
    ...
    for i in range(len(result)):
        ...
        for epoch in range(1, epochnum+1):
            ...
            loss = loss_func(pred_probas[:,0], labels)+ l2_norm * l2_lambda
        ...
        with torch.no_grad():
            ...
            labels = torch.tensor(y_test).long().to(device)
            # forward pass
            pred_probas = model(features)
        ...
        #torch.save(model.state_dict(), f"../../../models/test5/{task}/{SEED}_{i}net.pth")

with open((f"../../../../test_logs/test5/{task}/auroc.txt"), "w") as f:
    for auroc in metrics["auroc"]:
        f.write("%f\n" % auroc)
with open((f"../../../../test_logs/test5/{task}/aupr.txt"), "w") as f:
    for aupr in metrics["aupr"]:
        f.write("%f\n" % aupr)
with open((f"../../../../test_logs/test5/{task}/precision.txt"), "w") as f:
    for precision in metrics["precision"]:
        f.write("%f\n" % precision)
with open((f"../../../../test_logs/test5/{task}/recall.txt"), "w") as f:
    for recall in metrics["recall"]:
        f.write("%f\n" % recall)
with open((f"../../../../test_logs/test5/{task}/f1.txt"), "w") as f:
    for f1 in metrics["f1"]:
        f.write("%f\n" % f1)
with open((f"../../../../test_logs/test5/{task}/acc.txt"), "w") as f:
