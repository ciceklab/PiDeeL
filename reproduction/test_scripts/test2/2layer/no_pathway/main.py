...
for p in range(10):
    ...
    this_hamming= 0.0
    for row in range(pathway_info.shape[0]):
        this_hamming += (hamming(pathway_info[row], non_pathway_info[row]))
    distances.append(this_hamming)
    imp_con = np.sum(non_pathway_info[3]) + np.sum(non_pathway_info[13]) + np.sum(non_pathway_info[14])+ np.sum(non_pathway_info[33])+ np.sum(non_pathway_info[7])
    imp_cons.append(imp_con)
    for SEED in SEEDS:
        ...
        for i in range(len(result)):
            ...
            pathway_info_to = torch.as_tensor(non_pathway_info).to(device)
            model = Net(X_train.shape[1],1,pathway_info_to).to(device)
...
with open(f"../../../../test_logs/test2/{task}/distances.txt" ,"w") as f:
    for distance in distances:
        f.write("%f\n" % distance)

with open(f"../../../../test_logs/test2/{task}/c_indices.txt" ,"w") as f:
    for c_index in c_indices:
        f.write("%f\n" % c_index)

with open(f"../../../../test_logs/test2/{task}/imp_cons.txt" ,"w") as f:
    for imp_con in imp_cons:
        f.write("%f\n" % imp_con)