...
non_pathway_info = np.copy(pathway_info)
for p in range(10):
    rng.shuffle(non_pathway_info)
    this_hamming= 0.0
    ...
    for SEED in SEEDS:
        ...
        for i in range(len(result)):
            ...
            non_pathway_info_to = torch.as_tensor(non_pathway_info).to(device)
            model = Net(X_train.shape[1],1,non_pathway_info_to).to(device)
...
with open(f"../../../../test_logs/test14/{task}/distances.txt" ,"w") as f:
    for distance in distances:
        f.write("%f\n" % distance)

with open(f"../../../../test_logs/test14/{task}/c_indices.txt" ,"w") as f:
