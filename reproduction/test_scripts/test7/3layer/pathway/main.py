...
from load_targeted_data import predicted_quant, events, survival,grade,pathway_info
from model import Net, initialize_weights, Net2
from config import learningrate, epochnum, task, SEEDS,patience,l2_lambda, l1weight,l2weight
import pdb
...
for SEED in SEEDS:
    ...
    for i in range(len(result)):
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        #plt.savefig(f"../../../plots/test7/{task}/{SEED}_{i}.png")
        #plt.clf()
        """
        with torch.no_grad():
        ...
        pretrained_dict["fc3.weight"] = torch.reshape(pretrained_dict["fc3.weight"][0],(1,64))
        pretrained_dict["fc3.bias"] = torch.reshape(pretrained_dict["fc3.bias"][0],(1,))
        ...
        for s in range(len(labels_test)):
            print(f"duration = {labels_test[:,1][s]:.0f}, event = {labels_test[:,0][s]:.0f}, grade = {labels2_test[s]:.0f}, risk score = {model(features_test)[:,0][s]:.3f}, predicted grade =  {F.sigmoid(model(features_test)[:,1][s]):.0f}")
        ...
        #model.save_net(f"../../../models/{task}/{SEED}_{i}.pth")
        #torch.save(net.state_dict(), f"../../../models/{task}/{SEED}_{i}net.pth")
        """
        #torch.save(model.state_dict(), f"../../../models/test7/{task}/{SEED}_{i}net.pth")


with open(f"../../../../test_logs/test7/{task}/c_indices.txt" ,"w") as f:
    for c_index in c_indices:
        f.write("%f\n" % c_index)
with open((f"../../../../test_logs/test7/{task}/auroc.txt"), "w") as f:
    for auroc in metrics["auroc"]:
        f.write("%f\n" % auroc)
with open((f"../../../../test_logs/test7/{task}/aupr.txt"), "w") as f:
    for aupr in metrics["aupr"]:
        f.write("%f\n" % aupr)
with open((f"../../../../test_logs/test7/{task}/precision.txt"), "w") as f:
    for precision in metrics["precision"]:
        f.write("%f\n" % precision)
with open((f"../../../../test_logs/test7/{task}/recall.txt"), "w") as f:
    for recall in metrics["recall"]:
        f.write("%f\n" % recall)
with open((f"../../../../test_logs/test7/{task}/f1.txt"), "w") as f:
    for f1 in metrics["f1"]:
        f.write("%f\n" % f1)
with open((f"../../../../test_logs/test7/{task}/acc.txt"), "w") as f:
