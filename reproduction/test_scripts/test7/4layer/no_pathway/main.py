...
from load_targeted_data import predicted_quant, events, survival,grade
...
for SEED in SEEDS:
    ...
    for i in range(len(result)):
        ...
        model = Net(X_train.shape[1],2).to(device)
        ...
        for epoch in range(1, epochnum+1):
            ...
            loss = loss1 + loss2+ (l2_norm* l2_lambda)
            #print(f"loss1 = {loss1}, loss2 ={5*loss2}")
            ...
            # report losses
            #print(f"Repeat {times+1}, Epoch {epoch}, Train loss: {train_losses[-1]}, Vald loss: {vald_losses[-1]}")
        ...
        net = Net2(X_train.shape[1],1).to(device)
        model1 = CoxPH(net, tt.optim.Adam)

        pretrained_dict["fc4.weight"] = torch.reshape(pretrained_dict["fc4.weight"][0],(1,64))
        pretrained_dict["fc4.bias"] = torch.reshape(pretrained_dict["fc4.bias"][0],(1,))
