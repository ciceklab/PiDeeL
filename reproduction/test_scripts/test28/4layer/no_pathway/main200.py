...
for k in range(10):
    np.random.seed(k)
    random_indices = np.random.choice(predicted_quant.shape[0], 200, replace=False)
    predicted_quant = predicted_quant[random_indices]
    events = events[random_indices]
    survival = survival[random_indices]
...
with open(f"../../../../test_logs/test28/{task}/c_indices_200.txt" ,"w") as f:
